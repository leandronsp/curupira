defmodule Mix.Tasks.BuildStatic do
  ...
  defp render_regular_article(article) do
    ...
    <a
      href="/articles/#{article.slug}.html"
      class="article-card group block p-6 bg-base-100 border border-transparent rounded-xl hover:shadow-lg hover:border-primary/20 transition-all cursor-pointer no-underline"
      data-slug="#{article.slug}"
      data-title="#{String.downcase(article.title)}"
      data-tags="#{String.downcase(Enum.join(article.tags || [], " "))}"
      data-language="#{article.language || "en"}"
